...
class DataProcessingStartPage(tk.Frame):
    def __init__(self, master,folderName,expNum,ex_data):
        ...
        cytCalibrationParametersButton = tk.Button(mainWindow,text='Enter CBA bead calibration parameters',command=lambda: master.switch_frame(cydp.CalibrationParameterPage,folderName,expNum,ex_data,DataProcessingStartPage))
        ...
        cellAbPanelButton = tk.Button(mainWindow,text='Edit antibody panel',command=lambda: master.switch_frame(cdp.MarkerNumberPage,folderName,expNum,ex_data,DataProcessingStartPage))
        ...
        prolifGenerationGatesButton = tk.Button(mainWindow,text='Edit generation gates',command=lambda: master.switch_frame(pdp.GatingPage,folderName,expNum,ex_data,DataProcessingStartPage))
        ...
        for i,button in enumerate([cytDfButton,cellDfButton,prolifDfButton,prolifGenerationGatesButton,singleCellDfButton,completeSingleCellDfButton]):
            if i == 0:
                requiredFiles = ['CBAcalibrationParameters-'+folderName+'.json','A1_cyt.csv']
            elif i == 1:
                requiredFiles = ['antibodyPanel-'+folderName+'.csv','A1_cell.csv']
            elif i == 2:
                requiredFiles = ['singleCellDataFrame-proliferation-'+folderName+'.pkl']
            elif i == 3:
                requiredFiles = ['logicleProliferationDf.pkl','rawProliferationDf.pkl','A1_cell.csv']
            elif i ==4:
...
def dataProcessingMaster(folderName,expNum,dataType,ex_data):
    ...
    levelLayouts = pickle.load(open('inputFiles/levelLayouts-'+folderName+'.pkl','rb'))
    ...
    if(dataType == 'cyt'):
        calibrationParameters = json.load(open('inputFiles/CBAcalibrationParameters-'+folderName+'.json','r'))
        ...
        if (('initialSingleCellDf-channel-'+folderName+'.pkl' not in fileList) or ('initialSingleCellDf-scale-'+folderName+'.pkl' not in fileList)):
            dataType = 'singlecell'
            fileNameDf = idp.createBaseDataFrame(experimentParameters,folderName,expNum,dataType,allColumnVariableCoordinates,experimentLevelLayoutDict)
