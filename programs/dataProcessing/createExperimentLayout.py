...
import numpy as np
import seaborn as sns
from miscFunctions import exitEnabledSubprocessRun,extractValues  
...
class plateGUIFrame(tk.Frame):
    def __init__(self, master,folderName,experimentParameters,parametersUpdatedByGridGUI,buttonImagesPathAndHexcolorsInput,homepage):
        ...
        actionButtons.pack()
        tk.Button(actionButtons, text="Next Level",command=grid.create_full_level_value_layout).pack(side=LEFT)
        tk.Button(actionButtons, text="Tile Horizontally",command=lambda:grid.tile_level('horizontal')).pack(side=LEFT)
        tk.Button(actionButtons, text="Tile Vertically",command=lambda:grid.tile_level('vertical')).pack(side=LEFT)
        def quitCommand():
            ...
            quit()
        tk.Button(actionButtons, text="Quit",command=lambda: quitCommand()).pack(side=LEFT)
