...
class ExperimentSetupStartPage(tk.Frame):
    def __init__(self, master,fName,path):
        ...
        tk.Button(mainWindow, text="Create experiment",command=lambda: createExperimentFolders(folderName)).pack(fill=X)
        tk.Button(mainWindow, text="Input experiment parameters",command=lambda: master.switch_frame(ExperimentParameterPage,folderName)).pack(fill=X)
        def plateLayout():
            os.chdir(folderName)
        ...
        tk.Button(mainWindow, text="Create experiment layout",command=lambda: plateLayout()).pack(fill=X)
