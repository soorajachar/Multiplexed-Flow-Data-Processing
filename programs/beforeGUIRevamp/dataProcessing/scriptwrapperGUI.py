...
#import facetPlotLibrary_updatedGUI as fplgui
        ...
        if(int(scriptToRun/100.) == 0): #Initial data processing
            ...
            if(scriptToRun in [101,102,103,104,106,107,108,109,110,111,112,113,114]): #Facet plots (line,scatter,scatter w/line for expfits)
                for dfKey in dfArray:
                    figpath = '/'.join(cwd.split('/')[:-1]+['figuresPipeline'])
                    print(figpath)
                    print('holeefuk')
                    exitEnabledSubprocessRun('python3',figpath+'/facetPlotLibrary_updatedGUI.py','',False)
                    #fplgui.main()
