...
def createCBACorrectionParameters(inputExpNum,inputFolderName):
    ...
    with open('cbaCorrectionParameters-'+folderName+'.json','w') as f:
        json.dump([fullLevelList,names,timepoints],f)

def createCorrectedGFI(inputExpNum,folderName):
    fullLevelList = json.load(open('cbaCorrectionParameters-'+folderName+'.json','r'))[0]
    names = json.load(open('cbaCorrectionParameters-'+folderName+'.json','r'))[1]
    timepoints = json.load(open('cbaCorrectionParameters-'+folderName+'.json','r'))[2]

    os.chdir(pathToExperimentSpreadsheet+'/'+folderName)
    excel_data = pd.read_excel(pathToExperimentSpreadsheet+'masterExperimentSpreadsheet.xlsx')
    calibrateExperiment(folderName,'',1e9,'nM',excel_data['NumberOfCBAStandardDilutions'][inputExpNum-1],excel_data['CBAStandardDilutedVolume'][inputExpNum-1])
