...
from miscFunctions import reindexDataFrame,cleanUpFileList
...
def createProliferationSingleCellDataFrame(folderName,secondPath,experimentNumber,useModifiedDf):
    ...
    logicleDataProliferation = logicleDataFull.xs(['TCells'],level=['CellType'])
    rawDataFull = pickle.load(open('semiProcessedData/initialSingleCellDf-scale-'+folderName+'.pkl','rb'))['TCell_Gate']
    rawDataProliferation = rawDataFull.xs(['TCells'],level=['CellType'])

    manual=False
    plot=True
    proliferationProcesser.processProliferationData(folderName,logicleDataProliferation,rawDataProliferation,manual,plot)
