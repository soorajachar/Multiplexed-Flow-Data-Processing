...
def returnGatesAndTicks(logicleData,rawData,justGates):
    xtickValues = []
    tempraw = []
    templin = []
    ...
    if not justGates:
        ...
        logxticks = [-1000,1000,10000,100000]
        logicleData = logicleData.iloc[:,2:]
    ...
    for tickval in logxticks:
        ...
        unstackedcolumn = int(temprawtickindex /  logicleData.shape[0])
        unstackedrow = temprawtickindex - (unstackedcolumn*logicleData.shape[0])
        temprawlevels = rawData.iloc[unstackedrow,:].name
        templintick = logicleData.loc[temprawlevels,logicleData.columns[unstackedcolumn]]
        xtickValues.append(templintick)
    if justGates:
        maxlogicle = max(logicleData.values.ravel())
        minlogicle = min(logicleData.values.ravel())
        lowerticksindex = []
        upperticksindex = []
        for xtick,i in zip(xtickValues,range(len(xtickValues))):
            if xtick < minlogicle:
                lowerticksindex.append(i)
            elif xtick > maxlogicle:
                upperticksindex.append(i)
        if len(upperticksindex) <= 0 or len(lowerticksindex) <= 0: 
            if len(upperticksindex) <= 0:
                xtickmax = len(xtickValues)
                if len(lowerticksindex) <= 0:
                    xtickmin = 0
                else:
                    xtickmin = max(lowerticksindex)
            else:
                xtickmax = min(upperticksindex)
        else:
            xtickmin = max(lowerticksindex)
            xtickmax = min(upperticksindex)
        xtickValues = xtickValues[xtickmin:xtickmax]
        xtickLabels = xtickLabels[xtickmin:xtickmax]
